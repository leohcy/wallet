...
public class RecordDao extends DaoSupport<Record> {
	...
	public List<Record> query(CategoryType categoryType, Long category,
		...
		if (accountType != null || account != null) {
			criteria.createAlias("incomeAccount", "i", Criteria.LEFT_JOIN)
					.createAlias("outlayAccount", "o", Criteria.LEFT_JOIN)
					.createAlias("fromAccount", "f", Criteria.LEFT_JOIN)
					.createAlias("toAccount", "t", Criteria.LEFT_JOIN);
			if (accountType != null) {
